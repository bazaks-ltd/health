...
class InpatientRecord(Document):
    def after_insert(self):
        frappe.db.set_value("Patient", self.patient,
                            "inpatient_record", self.name)
        frappe.db.set_value("Patient", self.patient,
                            "inpatient_status", self.status)
        ...
        if self.admission_encounter:  # Update encounter
            ...
            medication_requests = frappe.get_all(
                "Medication Request", filters, ["name"])
            service_requests = frappe.get_all(
                "Service Request", filters, ["name"])
    ...
    def validate_dates(self):
        if (get_datetime(self.expected_discharge) < get_datetime(self.scheduled_date)) or (
            ...
            frappe.throw(
                _("Expected and Discharge dates cannot be less than Admission Schedule date"))
        ...
        for entry in self.inpatient_occupancies:
            if (
                ...
                frappe.throw(
                    _("Row #{0}: Check Out datetime cannot be less than Check In datetime").format(
                        entry.idx)
                )
...
def get_clinical_notes(patient, note_type=None):
    ...
    return frappe.get_all(
        "Clinical Note",
        filters=filters,
        fields=["posting_date", "note", "name",
                "practitioner", "employee", "clinical_note_type"],
...
def schedule_inpatient(args):
    ...
    if encounter and encounter.therapies:  # Therapies
        ...
        set_ip_child_records(
            inpatient_record, "therapies", encounter.therapies)
...
def validate_inpatient_invoicing(inpatient_record):
    ...
    if pending_invoices:
        ...
        frappe.throw(message, title=_("Unbilled Services"),
...
def get_pending_invoices(inpatient_record):
    ...
    docs = ["Patient Appointment", "Patient Encounter",
            "Lab Test", "Clinical Procedure"]

    for doc in docs:
        doc_name_list = get_unbilled_inpatient_docs(doc, inpatient_record)
        if doc_name_list:
            pending_invoices = get_pending_doc(
                doc, doc_name_list, pending_invoices)
...
def set_ip_order_cancelled(inpatient_record, reason, encounter=None):
    ...
    if inpatient_record.status == "Admission Scheduled":
        ...
        if encounter_name:
            frappe.db.set_value(
                "Patient Encounter", encounter_name, {
                    "inpatient_status": None, "inpatient_record": None}
            )
...
def validate_incompleted_service_requests(inpatient_record):
    ...
    service_requests = frappe.db.get_list(
        "Service Request", filters=filters, pluck="name")
