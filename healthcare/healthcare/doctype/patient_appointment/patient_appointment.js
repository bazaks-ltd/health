...
frappe.ui.form.on("Patient Appointment", {
  ...
  refresh: function (frm) {
    ...
    if (
      ["Open", "Checked In"].includes(frm.doc.status) ||
      ...
      if (frm.doc.procedure_template) {
        frm.add_custom_button(
          ...
          function () {
            frappe.model.open_mapped_doc({
              method:
                "healthcare.healthcare.doctype.clinical_procedure.clinical_procedure.make_procedure",
              frm: frm,
            });
          },
          __("Create")
        ...
        frm.add_custom_button(
          ...
          function () {
            frappe.model.open_mapped_doc({
              method:
                "healthcare.healthcare.doctype.therapy_session.therapy_session.create_therapy_session",
              frm: frm,
            });
          },
          "Create"
  ...
  toggle_payment_fields: function (frm) {
    frappe.call({
      method:
        "healthcare.healthcare.doctype.patient_appointment.patient_appointment.check_payment_reqd",
  ...
  get_prescribed_therapies: function (frm) {
    if (frm.doc.patient) {
      frappe.call({
        method:
          "healthcare.healthcare.doctype.patient_appointment.patient_appointment.get_prescribed_therapies",
        args: { patient: frm.doc.patient },
        callback: function (r) {
          if (r.message) {
            ...
            frappe.msgprint({
              ...
              message: __("There are no Therapies prescribed for Patient {0}", [
              ...
              frappe.call({
                ...
                callback: (r) => {
                  if (val && !r.message && !frm.doc.invoiced) {
                    ...
                    frappe.call({
                      method:
                        "healthcare.healthcare.doctype.patient_appointment.patient_appointment.update_fee_validity",
                      args: { appointment: frm.doc },
                    });
                  }
                },
    ...
    slot_details.forEach((slot_info) => {
      ...
      if (fee_validity && fee_validity != "Disabled") {
        slot_html += `
					...
					${__("Patient has fee validity till")} <b>${moment(
...
let get_prescribed_procedure = function (frm) {
  if (frm.doc.patient) {
    frappe.call({
      method:
        "healthcare.healthcare.doctype.patient_appointment.patient_appointment.get_procedure_prescribed",
      args: { patient: frm.doc.patient },
      callback: function (r) {
