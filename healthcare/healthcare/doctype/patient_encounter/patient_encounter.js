...
frappe.ui.form.on("Patient Encounter", {
  ...
  setup: function (frm) {
    ...
    if (
      frappe.meta.get_docfield("Drug Prescription", "medication")
        .in_list_view === 1
    ) {
  ...
  refresh: function (frm) {
    ...
    if (!frm.doc.__islocal) {
      if (frm.doc.docstatus === 1) {
        if (
          !["Discharge Scheduled", "Admission Scheduled", "Admitted"].includes(
            frm.doc.inpatient_status
          )
        ) {
        ...
        frm.add_custom_button(
          ...
          function () {
            frappe.model.open_mapped_doc({
              method:
                "healthcare.healthcare.doctype.patient_encounter.patient_encounter.make_ip_medication_order",
              frm: frm,
            });
          },
          __("Create")
    ...
    if (
      ...
      frm.set_query("drug_code", "drug_prescription", function (doc, cdt, cdn) {
        ...
        return {
          query:
            "healthcare.healthcare.doctype.patient_encounter.patient_encounter.get_medications_query",
          filters: filters,
        };
      });
    }
...
var schedule_inpatient = function (frm) {
  ...
  var dialog = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "healthcare.healthcare.doctype.inpatient_record.inpatient_record.schedule_inpatient",
        args: {
          args: args,
        },
        callback: function (data) {
...
let create_lab_test = function (frm) {
  if (!frm.doc.patient) {
    frappe.throw(__("Please select patient"));
  }
  let d = new frappe.ui.Dialog({
    ...
    primary_action(values) {
      frappe.route_options = {
        ...
        practitioner: frm.doc.practitioner,
        template_dn: values.lab_test_template,
        // 'medical_department': frm.doc.medical_department,
        // 'company': frm.doc.company
      };
...
let create_nursing_tasks = function (frm) {
  const d = new frappe.ui.Dialog({
    ...
    primary_action: () => {
      ...
      frappe.call({
        method:
          "healthcare.healthcare.doctype.nursing_task.nursing_task.create_nursing_tasks_from_template",
        args: {
...
let create_service_request = function (frm) {
  frappe.call({
    method:
      "healthcare.healthcare.doctype.patient_encounter.patient_encounter.create_service_request",
    freeze: true,
    args: {
      encounter: frm.doc.name,
    },
...
let create_medication_request = function (frm) {
  frappe.call({
    method:
      "healthcare.healthcare.doctype.patient_encounter.patient_encounter.create_medication_request",
    freeze: true,
    args: {
      encounter: frm.doc.name,
    },
...
var apply_code_sm_filter_to_child = function (
  frm,
  field,
  table_list,
  code_system
