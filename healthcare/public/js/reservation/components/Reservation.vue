...
const props = defineProps(['resources', 'update'])
const events = ref([])
const vcal = ref(null);
...
const fetchEvents = () => {
    frappe.db.get_list("Patient Appointment", {
        fields: ["*"],
        filters: [
            ["service_unit", "in", props.resources.map(r => r.name)],
            ["appointment_datetime", ">=", vcal.value.view.startDate.toISOString()],
            ["appointment_datetime", "<=", vcal.value.view.endDate.toISOString()]
        ],
        limit: 0
    }).then(res => {
        events.value = res.map(e => ({
            id: e.name,
            title: "Pat: " + e.patient_name + "<br />" + (e.referring_practitioner ? "Ref: " + e.referring_practitioner : ""),
            start: new Date(e.appointment_datetime),
            end: new Date(new Date(e.appointment_datetime).getTime() + e.duration * 60 * 1000),
            split: e.service_unit,
        }))
    })
}
...
.vuecal__view-btn--highlighted:after {
    ...
    animation: sonar .8s infinite ease-out;
    position: absolute;
    top: 50%;
    left: 50%;
    pointer-events: none
