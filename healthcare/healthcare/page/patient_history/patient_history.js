...
class PatientHistory {
  ...
  show() {
    ...
    this.sidebar
      .find('[data-fieldname="patient"]')
      .append('<div class="patient-info"></div>');
  }
        ...
        let doctype_filter = frappe.ui.form.make_control({
          ...
          df: {
            ...
            change: () => {
              ...
              this.setup_documents(
                doctype_filter.get_value(),
                date_range_field.get_value()
              );
            },
            get_data: () => {
  ...
  setup_documents(document_types = "", selected_date_range = "") {
    ...
    if (document_types) filters["document_types"] = document_types;
    if (selected_date_range) filters["date_range"] = selected_date_range;

    let me = this;
    frappe.call({
      method:
        "healthcare.healthcare.page.patient_history.patient_history.get_feed",
      args: filters,
      callback: function (r) {
        let data = r.message;
        if (data.length) {
    ...
    for (i = 0; i < data.length; i++) {
      if (data[i].reference_doctype) {
        ...
        if (frappe.user_info(data[i].owner).image) {
          data[i].imgsrc = frappe.utils.get_file_link(
            frappe.user_info(data[i].owner).image
          );
        } else {
          data[i].imgsrc = false;
        }

        let time_line_heading = data[i].practitioner
          ? `${data[i].practitioner} `
          : ``;
        time_line_heading +=
          data[i].reference_doctype +
          " - " +
  ...
  show_patient_info() {
    this.get_patient_info().then(() => {
      $(".patient-info")
        .empty()
        .append(
          frappe.render_template("patient_history_sidebar", {
  ...
  show_patient_details() {
    ...
    frappe.call({
      method:
        "healthcare.healthcare.doctype.patient.patient.get_patient_detail",
      args: {
      ...
      callback: function (r) {
        ...
        if (data.occupation)
          details += `<br><br><b> ${__("Occupation")} : </b> ${
            data.occupation
          }`;
        if (data.blood_group)
          details += `<br><b> ${__("Blood Group")} : </b> ${data.blood_group}`;
        if (data.allergies)
          details += `<br><br><b> ${__(
            "Allerigies"
          )} : </b> ${data.allergies.replace(/\n/g, ", ")}`;
        if (data.medication)
          details += `<br><b> ${__(
            "Medication"
          )} : </b> ${data.medication.replace(/\n/g, ", ")}`;
        if (data.alcohol_current_use)
          details += `<br><br><b> ${__("Alcohol use")} : </b> ${
            data.alcohol_current_use
          }`;
        if (data.alcohol_past_use)
          details += `<br><b> ${__("Alcohol past use")} : </b> ${
            data.alcohol_past_use
          }`;
        if (data.tobacco_current_use)
          details += `<br><b> ${__("Tobacco use")} : </b> ${
            data.tobacco_current_use
          }`;
        if (data.tobacco_past_use)
          details += `<br><b> ${__("Tobacco past use")} : </b> ${
            data.tobacco_past_use
          }`;
        if (data.medical_history)
          details += `<br><br><b> ${__(
            "Medical history"
          )} : </b> ${data.medical_history.replace(/\n/g, ", ")}`;
        if (data.surgical_history)
          details += `<br><b> ${__(
            "Surgical history"
          )} : </b> ${data.surgical_history.replace(/\n/g, ", ")}`;
        if (data.surrounding_factors)
          details += `<br><br><b> ${__(
            "Occupational hazards"
          )} : </b> ${data.surrounding_factors.replace(/\n/g, ", ")}`;
        if (data.other_risk_factors)
          details += `<br><b> ${__(
            "Other risk factors"
          )} : </b> ${data.other_risk_factors.replace(/\n/g, ", ")}`;
        if (data.patient_details)
          details += `<br><br><b> ${__(
            "More info"
          )} : </b> ${data.patient_details.replace(/\n/g, ", ")}`;

        if (details) {
          details =
            `<div style='font-size:13px;' align='left'>` + details + `</div>`;
        }
  ...
  setup_buttons() {
    ...
    this.page.main.on("click", ".btn-show-chart", function () {
      let btn_id = $(this).attr("data-show-chart-id"),
        scale_unit = $(this).attr("data-pts");
...
frappe.patient_history.PatientHistory = PatientHistory;
