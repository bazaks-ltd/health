...
`;
    ...
    if (frm.doc.status === "Discharged") {
      ...
      // Disable "Add Doctors Encounter" and "Add Nurses Notes" buttons specifically
      // These button IDs are generated by UIPCForm using btoa(virtual_dt)
      const doctorsEncounterBtnId = btoa("Doctors Encounter").replace(/=/g, "");
      const nursesNotesBtnId = btoa("Nurses Notes").replace(/=/g, "");

      // Use setTimeout to ensure buttons are rendered by UIPCForm before disabling
      setTimeout(() => {
        const $doctorsBtn = $wrapper.find(`#${doctorsEncounterBtnId}`);
        const $nursesBtn = $wrapper.find(`#${nursesNotesBtnId}`);

        // Disable the Add buttons
        $doctorsBtn.prop('disabled', true);
        $nursesBtn.prop('disabled', true);

        // Remove click handlers to prevent any bypass
        $doctorsBtn.off('click');
        $nursesBtn.off('click');

        // Also disable any Edit/Delete buttons that may appear in the notes list
        $wrapper.find('#v-pills-doctors .btn-info, #v-pills-doctors .btn-danger').prop('disabled', true).off('click');
        $wrapper.find('#v-pills-nurses .btn-info, #v-pills-nurses .btn-danger').prop('disabled', true).off('click');
      }, 100);
    }
  },
